...
class StockRule(models.Model):
    ...
    def _run_buy(self, procurements):
        ...
        for procurement, rule in procurements:
            origin_sale_order = self.env['sale.order'].search([("name", '=', procurement.origin)])
            # Get the schedule date in order to find a valid seller
            procurement_date_planned = fields.Datetime.from_string(procurement.values['date_planned'])
            supplier = False
            if custom_dropship_enabled and len(origin_sale_order) == 1:
                if len(origin_sale_order.supplier_id) == 1:
                    custom_partner_id = origin_sale_order.supplier_id.id
                    supplier = self.env['product.supplierinfo'].search([('partner_id', '=', custom_partner_id)])
            elif procurement.values.get('supplierinfo_id'):
